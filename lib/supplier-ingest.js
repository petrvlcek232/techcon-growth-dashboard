...
function mapSupplierColumns(headers) {
    const mapping = {};
...
async function parseSupplierExcelFile(filePath) {
    try {
...
async function listSupplierFiles(dir) {
    try {
        const files = await readdir(dir);
        return files.filter(file => /\.(xls|xlsx)$/i.test(file));
    }
    catch (error) {
...
function aggregateSupplierData(rows) {
...
export async function ingestSupplierDirectory(dir = SUPPLIER_DATA_DIR) {
